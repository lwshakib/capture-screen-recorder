...
export default function UploadApp() {
  ...
  // Use refs to ensure proper cleanup and prevent duplicate listeners
  const hasSetupListener = useRef(false);
  const isProcessingUpload = useRef(false);
  const processedRecordings = useRef(new Set<string>());
  ...
  const handleUpload = useCallback(
    async (videoBlob: Blob, filename?: string) => {
      ...
      try {
        ...
        // Open the video URL in the web app
  ...
  React.useEffect(() => {
    ...
    const handleRecordingData = (
      ...
      if (data && data.type === "video-blob") {
        // Convert the array of numbers back to a Blob
    ...
    return () => {
      ...
      // Clear processed recordings on cleanup
